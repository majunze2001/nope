// asn1-parser.js -- https://git.coolaj86.com/coolaj86/asn1-parser.js
let ASN1 = {};
let Enc = {};
...
ASN1.parse = function parseAsn1Helper(buf) {
  ...
  function parseAsn1(buf, depth, eager) {
    ...
    // console.log(depth.join(ws) + '0x' + Enc.numToHex(asn1.type), index, 'len:', asn1.length, asn1);
...
ASN1.decode = function(o) {
  if (o === undefined){
    return {};
  }
  const parseArrayToHexStr = arr => {
    return arr.map(num => num.toString(16)).join("");
  };
  if (o.type === 12) {
    // UTF8String
    const result = String.fromCharCode(...o.value);
    return { UTF8String: result };
  } else if (o.type === 2) {
    // Integer
    return { INTEGER: parseInt(parseArrayToHexStr(o.value, 16)) };
  } else if (o.type === 3) {
    return { "BIT STRING": parseArrayToHexStr(o.value) };
  } else if (o.type === 4) {
    return { "OCTET STRING": String.fromCharCode(...o.value) };
  } else if (o.type === 130) {
    // return str directly
    return ASN1.octetArrayToString(o.value);
  } 
  else if (o.type === 48) {
    const vals=o.children.map(c => ASN1.decode(c));
    return vals;
  } else {
    return { TYPE: o.type, VAL: o.value };
  }
};

// Convert octet array to a UTF-8 string, to parse the proof
ASN1.octetArrayToString = octetArray => {
  return new TextDecoder().decode(new Uint8Array(octetArray));
};
