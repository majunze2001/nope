...
// used when the TLD is using RSA keys
// and when the SLD has an ECDSA KSK
template NopeRSAECDSAManaged() {
  // standard constants
  ...
  signal input tld_dnskey_sig[MAX_RSA_SIG_LEN];
  signal input tld_dnskey_sig_len;
  signal input tld_ksk[MAX_TLD_NAME_LEN + 4 + MAX_RSA_KEY_LEN];
  ...
  signal input sld_dnskey_sig[ECDSA_SIG_LEN];
  ...
  signal input sld_txt_sig[ECDSA_SIG_LEN];
  signal input sld_txt_key[ECDSA_KEY_LEN];
  ...
  component HashTLDKSK = SHA256(MAX_TLD_NAME_LEN + 4 + MAX_RSA_KEY_LEN);
  HashTLDKSK.msg <== tld_ksk;
  HashTLDKSK.real_byte_len <== tld_ksk_len;
  component TLDDSVerify = VerifyDSSuffixFromHashRSA(MAX_RSA_SIG_LEN, MAX_RSA_KEY_LEN, 8);
  ...
  component TLDKSKExtract = ExtractKSK(MAX_TLD_NAME_LEN, MAX_RSA_KEY_LEN, 8);
  TLDKSKExtract.ksk <== tld_ksk;
  TLDKSKExtract.real_ksk_byte_len <== tld_ksk_len;
  TLDKSKExtract.real_name_byte_len <== tld_name_len;
  component TLDDNSKEYVerify = VerifyDNSKEYRSA(MAX_TLD_DNSKEY_REC_LEN, MAX_RSA_SIG_LEN, MAX_RSA_KEY_LEN, MAX_TLD_NAME_LEN);
  TLDDNSKEYVerify.rec <== tld_dnskey_rec;
  TLDDNSKEYVerify.real_rec_byte_len <== tld_dnskey_rec_len;
  TLDDNSKEYVerify.sig <== tld_dnskey_sig;
  TLDDNSKEYVerify.real_sig_byte_len <== tld_dnskey_sig_len;
  TLDDNSKEYVerify.ksk <== TLDKSKExtract.key;
  TLDDNSKEYVerify.real_ksk_byte_len <== TLDKSKExtract.real_key_byte_len;
  TLDDNSKEYVerify.zsk <== sld_ds_key;
  TLDDNSKEYVerify.real_zsk_byte_len <== sld_ds_key_len;
  ...
  component SLDDNSKEYVerify = VerifyDNSKEYECDSA(MAX_SLD_DNSKEY_REC_LEN, MAX_SLD_NAME_LEN);
  ...
  component SLDTXTVerify = VerifyTXTSuffixECDSA();
...
component main { public[packed_pub_inputs] } = NopeRSAECDSAManaged();
