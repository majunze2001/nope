...
template RSAfromECDSA() {
  ...
  signal input ksk[MAX_TLD_NAME_LEN + 4 + ECDSA_KEY_LEN];
  signal input ksk_len;
  // child ZSK
  signal input child_zsk[ECDSA_KEY_LEN];

  // DNSKEY parse and sig
  component KSKExtract = ExtractKSK(MAX_TLD_NAME_LEN, ECDSA_KEY_LEN, 13);
  KSKExtract.ksk <== ksk;
  KSKExtract.real_ksk_byte_len <== ksk_len;
  KSKExtract.real_name_byte_len <== child_name_len;
  component DNSKEYVerify = VerifyDNSKEYECDSA(MAX_TLD_DNSKEY_REC_LEN, MAX_TLD_NAME_LEN);
  ...
  // KSK hash
  component HashKSK = SHA256(MAX_TLD_NAME_LEN + 4 + ECDSA_KEY_LEN);
  HashKSK.msg <== ksk;
